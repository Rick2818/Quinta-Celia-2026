...
/**
 * Busca clientes en Supabase Cloud filtrando por nombre, DUI/cédula, teléfono o lote
 */
export async function buscarClientesEnSupabase(termino: string, config: SupabaseSettings): Promise<ClienteComprador[]> {
  if (!config.conectado || !config.url || !config.anonKey || !termino.trim()) return [];

  try {
    const cleanUrl = config.url.trim().replace(/\/+$/, '');
    const q = encodeURIComponent(`*${termino.trim()}*`);
    const endpoint = `${cleanUrl}/rest/v1/${config.tableNameClientes || 'clientes_quinta_celia'}?or=(nombre.ilike.${q},cedula.ilike.${q},lote_nombre.ilike.${q},telefono.ilike.${q})&order=creado_en.desc&limit=25`;
